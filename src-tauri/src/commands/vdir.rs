...
/// `vdir` is equivalent to `ls -l -b` per GNU coreutils.
...
pub fn execute(
    ...
    if args.contains(&"--help") || args.contains(&"-h") {
        return Ok(r#"vdir - Linuxcommand.org Standard Parity
...
' with -q or -b"#.to_string());
    }

    // Delegate to ls with all flag handling
    super::ls::execute("vdir", args, stdin, st)
}
